...
var SymmetricKey;
var rsa;
function ecbblowfish(){
this.ecb_key=new ecb();
this.ecb_pass=new ecb();
this.CryptoKeyLength="";
this.EncryptedKey="";
this.SymmetricKey="";
this.rsa="";
}
ecbblowfish.prototype.setRemote = function(flg){
	return true;
}
ecbblowfish.prototype.internalTest = function(kg){
	return true;
}
ecbblowfish.prototype.setKey = function(key){

	this.ecb_key.setkey(key);
	this.SymmetricKey = this.generateKey(this.CryptoKeyLength);
	this.EncryptedKey = this.ecb_key.encrypt(this.SymmetricKey);
	this.ecb_pass.setkey(this.SymmetricKey);
}

ecbblowfish.prototype.setSymKey = function(key){

	this.ecb_key.setkey(key);
	this.SymmetricKey = key;
	this.ecb_pass.setkey(key);
}

ecbblowfish.prototype.setEnKey = function(key){

	this.EncryptedKey = key;
}

ecbblowfish.prototype.encrypt= function(str){
	return this.ecb_pass.encrypt(str);
}
ecbblowfish.prototype.decrypt= function(str){
	return this.ecb_pass.decrypt(str);
}
ecbblowfish.prototype.internalTest= function(str){
	return true;
}
ecbblowfish.prototype.setCryptoKeyLength= function(length){
	this.CryptoKeyLength = length;
}
ecbblowfish.prototype.getEncryptedKey= function(){
	return this.EncryptedKey;
}
ecbblowfish.prototype.getSymmetricKey= function(){
	return this.SymmetricKey;
}
ecbblowfish.prototype.setRemote= function(bool){
	return true;
}
ecbblowfish.prototype.encryptWithRSA= function(modulus, pas01){
	this.rsa = new RSAKey();
	this.rsa.setPublic(modulus, '10001');
	var res = this.rsa.encrypt(pas01);
	return res;
}
ecbblowfish.prototype.generateKey= function(len){
	var genKey ='';
	for(var i =0;i<len;i++){
		var NumChar = Math.floor(Math.random()*10)+'';
		NumChar = NumChar.substring(0, 1);
		genKey+=NumChar;
	}
	return genKey;
}
...
ecb.prototype.setkey = function (k)
{
	var nLen = k.length;
	if(nLen > 28)
	{
	...
	{
		var cActChar = k.charCodeAt(nI);
		bkey[nPos++] = (cActChar >> 8 & 0xff);
		bkey[nPos++] = (cActChar & 0xff);
	}

	this.strKey = this.byteArr2Str(bkey);
	this.bf = new Blowfish(this.strKey);
	delete bkey;
}

ecb.prototype.encrypt = function (sPlainText)
{	
	sPlainText = this.trim(sPlainText);
	var nLen = sPlainText.length
	var nOrigLen = nLen;
	if((nLen & 3) != 0){
		nLen = (nLen & -4) + 4;
	}
	...
	for(var nI = 0; nI < nLen; nI++){
		var cActChar;
		if(nI < nOrigLen){
			cActChar = sPlainText.charCodeAt(nI);
		} else{
			cActChar = ' '.charCodeAt(0);
		}
	buf[nPos++] = (cActChar >> 8 & 0xff);
	buf[nPos++] = (cActChar & 0xff);
	}
	var strPass = this.byteArr2Str(buf);
	return this.bf.encrypt(strPass);
}


ecb.prototype.decrypt = function (sCipherText)
{

	var res1 = new String(this.bf.decrypt(sCipherText));
	var nLen = res1.length;
	var res ='';
...
ecb.prototype.trim= function(s)
{
	return this.rtrim(this.ltrim(s));
}

ecb.prototype.ltrim= function(s)
{
	var l=0;
...
ecb.prototype.rtrim= function(s)
{
	var r=s.length -1;
	while(r > 0 && s[r] == ' ')
	{	r-=1;	}
...
ecb.prototype.byteArr2Str= function(arr)
{
	var re = '';
	for(var i = 0; i<arr.length; i++){
		re+=String.fromCharCode(arr[i]);
	}
	return re;
}
